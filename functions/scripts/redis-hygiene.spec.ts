 ...
 *
 * @param {object} options - Fixture inputs for the fake client.
 * @param {Record<string, number>} options.keys - Key to TTL (-1 = no expiry, -2 = gone).
 * @param {number} [options.backlog] - LLEN result reported for the analytics list.
 * @param {number} [options.zcard] - ZCARD result reported for the presence sets.
 * @return {AuditRedis} A client satisfying the audit's structural contract.
 */
