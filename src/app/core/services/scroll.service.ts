...
import { Inject, Injectable, OnDestroy, Renderer2, RendererFactory2, inject, DOCUMENT } from '@angular/core'
...
export class ScrollService implements OnDestroy {
  ...
  private _window = inject(WindowToken)
  ...
  supportManualScrollRestoration: boolean = !!this._window && ('scrollTo' in this._window) &&
    ('pageXOffset' in this._window) && isScrollRestorationWritable()
  ...
  constructor(
    ...
    fromEvent(this._window, 'beforeunload')
      .pipe(takeUntil(this.onDestroy))
    ...
    if (this._window.location.href !== this.getStoredScrollLocationHref()) {
      this.removeStoredScrollInfo()
    }
  }
  ...
  scrollToElement(element: HTMLElement | null) {
    if (element) {
      ...
      if (this._window && this._window.scrollBy) {
        ...
        //  element.)
        this._window.scrollBy(0, element.getBoundingClientRect().top - this.topOffset)

        // If we are very close to the top (<20px), then scroll all the way up.
        // (This can happen if `element` is at the top of the page, but has a small top-margin.)
        if (this._window.scrollY < 20) {
          this._window.scrollBy(0, -this._window.scrollY)
        }
      }
    }
  }

  scrollToElementByOffset(element: HTMLElement | null, offset: number = 4 * 16) {
    if (element) {
      // element.scrollIntoView()
      element.focus?.()

      if (this._window && this._window.scrollTo) {
        const rect = element.getBoundingClientRect()
        const scrollTop = this._window.scrollY
        const topOffset = rect.top + scrollTop - offset; // 4rem = 4 * 16px
        // this._window.scrollBy(0, rect.top + topOffset)
        this._window.scrollTo({ top: topOffset, behavior: 'smooth' })
  ...
  updateScrollLocationHref(): void {
    this.storage.setItem('scrollLocationHref', this._window.location.href)
  }
